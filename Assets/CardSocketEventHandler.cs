...
public class CardSocketEventHandler : MonoBehaviour
{
    ...
    */
    ...
    private void OnHoverEnter(HoverEnterEventArgs args)
    {
        ...
        if (socket.hasSelection && socket.interactablesSelected[0].transform.name != args.interactableObject.transform.name )
        {
            Debug.Log("Socket already has an object attached!");
            // Get the first interactable in the list
            var interactable = socket.interactablesSelected[0];
            if (interactable != null)
            {
                Debug.Log("Socket contains: " + interactable.transform.name);
            }            
        }
        else
        {
